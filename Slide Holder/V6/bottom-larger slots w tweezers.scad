...
module coverslip(){
    translate([4.5+13.5,0,0])rotate([0,90,0])cylinder(h=1, r=20, $fn=500);
}
//coverslip();

// Components for tweezers

module tweezer(){
    rotate([-7,0,0])union(){
        cube(size=[10, 1, 10]);
        translate([0,1,0])cube(size=[10,1.5,1]);
        translate([0,0,10])rotate([-30,0,0])cube(size=[10, 1, 47]);
    }
}
...
module tweezers(){
    union(){
        union(){
            tweezer();
            translate([10, 60, 0])rotate([0,0,180])tweezer();
        }
