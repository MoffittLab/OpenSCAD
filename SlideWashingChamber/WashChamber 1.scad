...
module case() {
    difference(){
        translate([0,0,0])cube(size=([48, 59, 45])); //Outside box 99
        translate([3,3,3])cube(size=([42, 53, 43])); // Inside 93
    }
    //Add ribs
    for (pos=[3:5:54]){
        union(){
            translate([0, pos, 0])cube(size=([6,3,43]));
            translate([42, pos, 0])cube(size=([6,3,43]));
        }
    }
   //Add lid holder and symmetry breaking mark
    ...
    translate([51,59/2-9/2,42*3/4-3/2])rotate([0,0,90])catch();
}
//case();
...
case();
translate([24,2.5,23])rotate([0,0,90])coverslip();
//washcontainer();
//translate([0,0,45])color("red")cover();
